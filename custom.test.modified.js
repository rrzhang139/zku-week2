...
describe('Custom Tests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
    ...
    const { tornadoPool, token, omniBridge } = await fixture()
    ...
    expect(tornadoPoolBalance).to.be.equal('20000000000000000') // the amount left: deposit - withdraw
  })
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
    ...
    // Bob withdraws all of his funds from the shielded pool in L2
    ...
    // withdraws all of his funds from the shielded pool in L1
    const aliceWithdrawAmount = utils.parseEther('0.07')
    const aliceAdress = ethers.utils.getAddress('0xE468d20D5c805A26bfB78bcE686AFaffa52f15A2')
    const amountAfterTransfer = aliceChangeUtxo.amount
    console.log(amountAfterTransfer.sub(aliceWithdrawAmount).toNumber())
    const aliceChangeUtxo2 = new Utxo({
      amount: amountAfterTransfer.sub(aliceWithdrawAmount),
      keypair: aliceKeypair,
    })
    await transaction({
      tornadoPool,
      ...
      recipient: aliceAdress,
      isL1Withdrawal: true,
    })

    // alice balance
    const aliceBalance = await token.balanceOf(aliceAdress)
